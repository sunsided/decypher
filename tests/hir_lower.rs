//! Integration tests for the HIR lowering pass (`cypher_rs::analyze`).
//!
//! These tests call [`cypher_rs::analyze`] on Cypher strings and verify the
//! shape of the resulting [`cypher_rs::hir::HirQuery`].

use cypher_rs::analyze;
use cypher_rs::hir::{
...
fn analyze_from_preparsed_query() {
    let query =
        cypher_rs::parse("MATCH (p:Person)-[:KNOWS]->(f) WHERE p.age > 18 RETURN f.name").unwrap();
...
fn analyze_str_and_query_produce_same_result() {
    ...
    let query = cypher_rs::parse(input).unwrap();
...
fn try_from_str_for_query() {
    ...
    let query = cypher_rs::Query::try_from("MATCH (n) RETURN n").unwrap();
...
fn try_from_str_for_query_invalid() {
    use std::convert::TryFrom;
    let result = cypher_rs::Query::try_from("INVALID !!!");
    assert!(result.is_err());
}
